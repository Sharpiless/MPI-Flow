...
model = model.eval()
images = sorted(os.listdir(os.path.join(opt.data_path, 'image_3')))
    
    
for i, img in enumerate(tqdm(images)):
    # load input
    img_path = os.path.join(opt.data_path, 'image_3', img)
    disp_path = os.path.join(opt.data_path, 'depth_nv_3', img[:-4]+'.png')
    image = image_to_tensor(img_path).cuda().half()  # [1,3,h,w]
    obj_masks = []
    for i in range(4):
        dst_img_path = os.path.join(opt.data_path, 'mask_3_multi', img[:-4]+'_mask_{:02d}.png'.format(i))
    ...
    render_3dphoto_dynamic_objects(
        ...
        None,
        single=True,
        data_path='/mnt/mmtech01/usr/liangyingping/dataset/testing_synz_objects/training/',
        name=img,
        extra='image_3',
        z_only=False,
        repeat=10
