...
def generate_random_pose(base_motions=[0.05, 0.05, 0.05]):
    ...
    cz = (random.random() * 0.1 + base_motions[2]) * scz
    camera_mot = [cx * 0.3, cy * 0.3, cz]
...
def render_3dphoto_dynamic(
    ...
    for idx in range(1):
        ...
        # flow_mix_color[obj_mask_np >= mask_thresh] = flow_color[
        #     obj_mask_np >= mask_thresh
        # ]
        ...
        depth_res = np.vstack(
            [depth * scale, depth_dync * scale, depth_mask * 255]
        ).astype(np.uint8)
        ...
        # visualization
        res1 = np.vstack([src_np, frame_np, frame_dync_np, frame_mix])
        res2 = np.vstack([inpainted, flow_color, flow_dync_color, flow_mix_color])
        ...
        cv2.imwrite(os.path.join(data_path, "image.png"), res)
